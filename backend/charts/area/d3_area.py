...
def add_d3_area_option(response, chart, df, color):
...
def add_d3_area_description(response, chart, df, color, template):
    add_L1_description(response, chart, template['L1-info'])
    template = template['d3-area']
    
    if len(df.columns) == 2:
        add_univariate_d3_area_description(response, chart, df, color, template)
    else:
        add_multivariate_d3_area_description(response, chart, df, color, template)


def add_univariate_d3_area_description(response, chart, df, color, template):
    
    stats = df[df.columns[1]].describe()
...
def add_multivariate_d3_area_description(response, chart, df, color, template):
    ...
    for i, col in enumerate(df.columns[1:]):
        ...
        response['visualCue'][key_child] = [
            {'itemStyle': {'decal': config.DECAL}} if tmp_col == col else {} for tmp_col in df.columns[1:]
        ]
    ...
    schema = find_dict_item(response['schema'], key)
    schema['children'] = [
        {'title': {'en': 'stacked values', 'fr': 'valeurs empilées'}, 'key': 'L2-1-0-0'},
        {'title': {'en': 'category wise', 'fr': 'par catégorie'}, 'key': 'L2-1-0-1'}
    ]
    ...
    for i, col in enumerate(df.columns[1:]):
        idx, max_ = df_totals[col].idxmax(), df_totals[col].max().round(2)
        kw = [col, max_, df.iloc[idx,0]]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
        response['visualCue'][key].append({'data': [{'coord': [int(idx), max_], 'value': max_}]})
    ...
    response['visualCue'][key] = []
    for i, col in enumerate(df.columns[1:]):
        idx, max_ = df[col].idxmax(), df[col].max()
        kw = [col, max_, df.iloc[idx,0]]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
        response['visualCue'][key].append({'data': [{'coord': [int(idx), df_totals.iloc[idx,i+1]], 'value': max_}]})
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    schema = find_dict_item(response['schema'], key)
    schema['children'] = [
        {'title': {'en': 'stacked values', 'fr': 'valeurs empilées'}, 'key': 'L2-1-1-0'},
        {'title': {'en': 'category wise', 'fr': 'par catégorie'}, 'key': 'L2-1-1-1'}
    ]
    ...
    response['visualCue'][key] = []
    for i, col in enumerate(df.columns[1:]):
        idx, min_ = df_totals[col].idxmin(), df_totals[col].min().round(2)
        kw = [col, min_, df.iloc[idx,0]]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
        response['visualCue'][key].append({'data': [{'coord': [int(idx), min_], 'value': min_}]})
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    response['visualCue'][key] = []
    for i, col in enumerate(df.columns[1:]):
        idx, min_ = df[col].idxmin(), df[col].min()
        kw = [col, min_, df.iloc[idx,0]]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
        response['visualCue'][key].append({'data': [{'coord': [int(idx), df_totals.iloc[idx,i+1]], 'value': min_}]})
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    schema = find_dict_item(response['schema'], key)
    for i, col in enumerate(df.columns[1:]):
        mean_ = df_totals[col].mean().round(2)
        kw = [col, mean_]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    schema = find_dict_item(response['schema'], key)
    for i, col in enumerate(df.columns[1:]):
        std_ = df_totals[col].std().round(2)
        kw = [col, std_]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    schema = find_dict_item(response['schema'], key)
    for i, col in enumerate(df.columns[1:]):
        median_ = df_totals[col].median().round(2)
        kw = [col, median_]
        add_comparison(schema, col, col, key, i, template[key]['description']['multivariate'], kw, kw)
    add_description(response, key, template[key]['tag'], schema=schema)
    ...
    outlier_map = {}
    for i, col in enumerate(df.columns[1:]):
        outliers = get_outliers(df_totals[col].tolist())
        if len(outliers) > 0:
    ...
    correlation_map = {}
    for i, col in enumerate(df.columns[1:]):
        coefficient = pearsonr(np.arange(len(df)), df_totals[col].tolist())
        if coefficient[1] < 0.05:
    ...
    if len(correlation_map) == 0:
        ...
        for i, (col,correlation_type) in enumerate(correlation_map.items()):
            ...
            } if j <= i else {} for j in range(len(df.columns[1:]))]
    ...
    for i, col in enumerate(df.columns[1:]):
        ...
        if trend_type != 'non-monotonic':
            ...
            trend = get_trend(data)
            add_multivariate_trend(trend, df_totals, i, chart['type'], response, key_child, tag)